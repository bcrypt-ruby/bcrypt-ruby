...
#  define RUBY_1_9
	...
	static VALUE bcrypt_wrapper(void *_args) {
		...
		return (VALUE)rubybcrypt(args->output, args->key, args->salt);
	}
...
static VALUE bc_salt(VALUE self, VALUE cost, VALUE seed) {
	...
	rubybcrypt_gensalt(salt, icost, (uint8_t *)RSTRING_PTR(seed));
...
static VALUE bc_crypt(VALUE self, VALUE key, VALUE salt, VALUE cost) {
	...
	if (rubybcrypt(output, safeguarded, (char *)RSTRING_PTR(salt)) != NULL) {
		return rb_str_new2(output);
	} else {
		return Qnil;
	}
